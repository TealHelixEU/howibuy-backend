package eu.tealhelix.betterme.tests;
...
@QuarkusTestResource(value = PostgresAndKeycloakTestResource.class, initArgs = {
		...
		@ResourceArg(name = "pgConnectionDbUser", value = "th_betterme"),
		@ResourceArg(name = "pgConnectionDbPassword", value = "th_betterme"),
})
...
public class CorrelationIdWorkflowTest {
	...
	void testNewCorrelationId() {
		...
		String impersonationToken = RestAssured
				...
				.when()
				.post("/api/betterme/v1/tokenexchange")
				.then()
				.statusCode(200)
				.contentType(JSON)
				.extract()
